...
根据Batóg（2021）的做法，边际垂直$\beta$收敛模型的估计步骤分为两步。第一步，对式（16）进行估计，得到$\hat\alpha_1$，然后由$\hat\alpha_1$求得$\hat\beta$，其次将第$i$个地区从样本中删去，重复第一步，得到$\hat \beta_{i}^{n-1}$，再将两次得到的结果相减便得到了$i$地区的边际垂直收敛速度$\Delta\beta_i$，$\Delta\beta_i$的数学表达式为：
