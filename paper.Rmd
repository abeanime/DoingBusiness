...
前文基于$\beta$收敛模型的估计系数对全国层面和三大区域层面的收敛情况做出了分析。为能研究不同省市在在全国层面收敛过程中的异质性，本文近一步使用边际垂直$\beta$收敛模型展开研究。下图展示了29个省市边际垂直$\beta$系数的大小关系^[由于贵州省的边际垂直$\beta$系数过大，属于离群值，参照Batog（2021）的做法，本文未在下图中直接展示，而是进行单独的说明。]，其中左侧区域为2010年至2014年的情况，右侧区域为2015年至2019年的情况。横轴为具体省市，纵轴为边际垂直$\beta$系数的大小，上下虚型线表示均值加减标准差，而上下点型线表示均值加减两倍标准差，四条辅助线用来判断省市在收敛过程中的贡献方向和大小，并依次将所有省市划分为“较大正向贡献”、“一般正向贡献”、“无明显贡献”、“一般负向贡献”和“较大负向贡献”5种类型
...
为近一步展示省市在不同子考察期中贡献的动态变化，依照前后两个子考察期边际垂直$\beta$系数变化的绝对大小，按照从小到大的顺序做出了如图-？所示的哑铃图。其中浅色点表示第一个子考察内的边际垂直$\beta$系数，深色点表示第二个子考察期内的边际垂直$\beta$系数；虚线表示边际垂直$\beta$系数变大，实现表示变小。
